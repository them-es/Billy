...
use Billy\Mpdf\Conversion\DecToAlpha;
use Billy\Mpdf\Conversion\DecToCjk;
use Billy\Mpdf\Conversion\DecToHebrew;
use Billy\Mpdf\Conversion\DecToOther;
use Billy\Mpdf\Conversion\DecToRoman;
use Billy\Mpdf\Mpdf;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \Billy\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \Billy\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
