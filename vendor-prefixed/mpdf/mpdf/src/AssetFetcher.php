...
use Billy\Mpdf\File\LocalContentLoaderInterface;
use Billy\Mpdf\File\StreamWrapperChecker;
use Billy\Mpdf\Http\ClientInterface;
use Billy\Mpdf\Log\Context as LogContext;
use Billy\Mpdf\PsrHttpMessageShim\Request;
...
class AssetFetcher implements \Billy\Psr\Log\LoggerAwareInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \Billy\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \Billy\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if ($this->mpdf->debug) {
				throw new \Billy\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
